...
typedef nmath::graph::Graph THIS;

void THIS::add_edge_util(nmath::graph::VERT_S v0, nmath::graph::VERT_S v1, std::shared_ptr<nmath::graph::edge_data> const & edge_data)
{
	auto i = iter(v0);
...
void				THIS::add_edge(nmath::graph::VERT_S v0, nmath::graph::VERT_S v1)
{
...
nmath::graph::iterator::vert_graph	THIS::iter(nmath::graph::VERT_S v)
{
	auto it = vert_find(v);
...
nmath::graph::iterator::vert_graph		THIS::vert_find(nmath::graph::VERT_S v)
{
...
void				THIS::edge_erase(nmath::graph::VERT_S & v0, nmath::graph::VERT_S & v1)
{
...
/*
   ...
   */
void				THIS::distance_util(nmath::graph::VERT_S u)
{
	...
	for(auto it = u->edge_begin(); it != u->edge_end();) {
		graph::VERT_S const & v = it->_M_v1.lock();
...
void				THIS::distance(nmath::graph::VERT_S const & v0)
{
...
void				THIS::bridges_sub(nmath::graph::VERT_S const & n, int & t, std::vector<nmath::graph::edge> & ret)
{
	...
	for(auto i = n->edge_begin(); i != n->edge_end(); ++i) {
		nmath::graph::VERT_S const & v = i->_M_v1.lock();
...
void				THIS::components_util(nmath::graph::VERT_S const & u, int c)
{
	assert(u);
