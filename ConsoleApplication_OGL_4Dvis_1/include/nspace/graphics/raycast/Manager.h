#ifndef NSPACE_GRAPHICS_RAYCAST_MANAGER
#define NSPACE_GRAPHICS_RAYCAST_MANAGER
			...
			public:
				...
				*/
				...
				virtual void reload_buffer()
				{
					...
					_M_memobj_polytope->EnqueueWrite(app->_M_polytopes->_M_buffer, app->_M_polytopes->buffer_size);
				}
				...
				virtual void create_tasks_ray_face_intercept()
				{
					...
					for (unsigned int i = 0; i < w; ++i)
					{
						for (unsigned int j = 0; j < h; ++j)
						{
							...
							for (unsigned int k = 0; k < app->_M_polytopes->size(); ++k)
							{
								auto p = std::dynamic_pointer_cast<nmath::geometry::Polytope<M>>(app->_M_polytopes->operator[](k));
				...
				virtual void						render_init()
				{
					...
					unsigned int sz;
					// polytopes memobj
					_M_memobj_polytope = _M_ocl->create_buffer(CL_MEM_READ_WRITE, app->_M_polytopes->buffer_size);
					_M_memobj_polytope->EnqueueWrite(app->_M_polytopes->_M_buffer, app->_M_polytopes->buffer_size);
					...
					_M_memobj_test_polytope = _M_ocl->create_buffer(CL_MEM_READ_WRITE, app->_M_polytopes->buffer_size);
					_M_memobj_test_polytope->EnqueueWrite(app->_M_polytopes->_M_buffer, app->_M_polytopes->buffer_size);
				...
				virtual void						render2(unsigned char * pixelData)
				{
					...
					for (unsigned int j = 0; j < h; ++j)
					{
						for (unsigned int i = 0; i < w; ++i)
						{
							...
							auto polytope = std::dynamic_pointer_cast<nmath::geometry::Polytope<M>>(app->_M_polytopes->operator[](polytope_i));
							...
							for (unsigned int k = 0; k < app->_M_lights->size(); ++k)
							{


								std::shared_ptr<nspace::light::Point<M>> l = std::dynamic_pointer_cast<nspace::light::Point<M>>(app->_M_lights->operator[](k));
